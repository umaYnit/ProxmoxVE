...
if [[ -f ~/.openvino ]]; then
  ...
  $STD sudo --preserve-env=VIRTUAL_ENV,http_proxy,https_proxy -nu immich uv sync --extra openvino --active -n -p python3.11 --managed-python
  patchelf --clear-execstack "${VIRTUAL_ENV}/lib/python3.11/site-packages/onnxruntime/capi/onnxruntime_pybind11_state.cpython-311-x86_64-linux-gnu.so"
  msg_ok "Installed HW-accelerated machine-learning"
else
  msg_info "Installing machine-learning"
  $STD sudo --preserve-env=VIRTUAL_ENV,http_proxy,https_proxy -nu immich uv sync --extra cpu --active -n -p python3.11 --managed-python
